...
Both were caught by measuring the DOM rather than by looking, which is the argument for doing it.

**Stranded digits.** Mounting the digit conditionally inside `AnimatePresence` meant a key that reappeared mid-exit revived the outgoing element instead of replacing it, leaving old digits on screen. Exactly one child is now always mounted, with an empty placeholder standing in for a blank cell.

**A leak behind it.** Even then, outgoing spans survived at `opacity: 0` — invisible, but still real text for a screen reader to read out. The digit animation used a blur, and when a digit was replaced mid-entry the filter track never resolved, so `AnimatePresence` never heard it was safe to unmount. The blur is gone (transform and opacity only) and exits run on a fixed tween rather than a spring, which approaches rest instead of reaching it.

## References

[Rauno Freiberg's interface checklist](https://github.com/raunofreiberg/interfaces) for the input, focus and motion-duration rules, and [input-otp](https://input-otp.rodz.dev) for the single-native-input architecture.
